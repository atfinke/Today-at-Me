...
function spotifyPlaylistAddButtonClicked() {
    ...
    xhr.open("POST", '/spotify/add_track?now_playing_track_uri=' + nowPlayingTrackURI + '&=selected_playlist_uri=' + selectedPlaylist.value, true);
...
function spotifyPlaylistRemoveButtonClicked() {
    ...
    let xhr = new XMLHttpRequest();
    xhr.open("POST", '/spotify/remove_track?now_playing_track_uri=' + nowPlayingTrackURI + '&now_playing_playlist_uri=' + nowPlayingPlaylistURI, true);
    xhr.onreadystatechange = function () {
        if (this.readyState != 4) return;
        if (this.status == 200) {
...
function spotifyGetNowPlaying() {
    ...
    xhr.onreadystatechange = function () {
        ...
        if (this.status == 200) {
            ...
            document.getElementById('now-playing-playlist').innerHTML = response['pn'];
            let metadata = document.getElementById('now-playing-metadata')
