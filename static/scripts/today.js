...
function updateCountdownComponent(id, date) {
    ...
    element.getElementsByClassName("color-countdown-days")[0].innerHTML = paddedStringForNum(days);
    element.getElementsByClassName("color-countdown-hours")[0].innerHTML = paddedStringForNum(hours);
    element.getElementsByClassName("color-countdown-min")[0].innerHTML = paddedStringForNum(minutes);
    element.getElementsByClassName("color-countdown-sec")[0].innerHTML = paddedStringForNum(seconds);
}
...
function updateContainerRowDetails() {
    ...
    const details = document.getElementsByClassName("component-container-tableview-row-detail");
    for (let element of details) {
        ...
        if (nowTime - endDate > 0) {
            ...
            const seconds = Math.floor((distance % (1000 * 60)) / 1000);
            element.innerHTML = "T-" + minutes + ":" + paddedStringForNum(seconds);
            updateContainerRowDetailColor(element, "container-row-detail-blue");
        } else if (
...
function updateWeather() {
    ...
    for (let element of elements) {
        ...
        fetch(url)
            ...
            .catch(error => {
                element.getElementsByClassName("container-single-row-detail")[0].innerHTML = "-";
...
function lastfmRowClicked(element_id) {
    ...
    xhr.open("POST", "/spotify/play_track?track_name=" + song + "&artist=" + artist, true);
...
function spotifyPlaylistSelectionChanged() {
    document.getElementById("spotify-playlist-selection-button").innerHTML = "ADD";
}

function spotifyPlaylistAddButtonClicked() {
    document.getElementById("spotify-add-to-playlist").style.opacity = 0.5;

    let playlistURISelection = document.getElementById("spotify-playlist-selection");
    let addButton = document.getElementById("spotify-playlist-selection-button");

    let selectedPlaylist = playlistURISelection.options[playlistURISelection.selectedIndex];
    let nowPlayingTrackURI = document.getElementById("now-playing-metadata").dataset.trackUri;
...
function spotifyGetNowPlaying() {
    ...
    xhr.onreadystatechange = function() {
        ...
        if (this.status == 200 && "tn" in response && response["tn"] != undefined) {
            if (metadata.dataset.trackUri != response["turi"] || metadata.dataset.playlistUri != response["puri"]) {
            ...
            imageElement.src =
                "/spotify/now_playing.jpeg?destination=" +
                imageElement.dataset.destination +
                "&uri=" +
                response["turi"];
...
function windowResized() {
    ...
    if (width < 1000) {
        ...
        document.documentElement.style.setProperty("--column-margin", margin + "px");
        document.documentElement.style.setProperty("--top-column-margin", topMargin + "px");
    } else {
        document.documentElement.style.setProperty("--column-margin", normalMargin + "px");
        document.documentElement.style.setProperty("--top-column-margin", normalMargin / 2 + "px");
    }
}
...
function clearCache() {
    let xhr = new XMLHttpRequest();
    xhr.open("POST", "/clear_cache", true);
    xhr.onreadystatechange = function() {
        if (this.readyState != 4) return;
        if (this.status == 200) {
