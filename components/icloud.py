...
def fetch_devices():
    my_devices = [d for d in api.devices if d[config.ICLOUD_DEVICE_NAME_KEY].startswith(config.ICLOUD_DEVICE_NAME_PREFIX) and d[config.ICLOUD_DEVICE_TYPE_KEY] not in ICLOUD_FILTERED_DEVICE_TYPES]
    my_devices = sorted(my_devices, key = lambda i: i[config.ICLOUD_DEVICE_NAME_KEY]) 
